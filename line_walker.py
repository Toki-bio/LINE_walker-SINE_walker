...
def _try_extended_flanks(step_num, tag, prev_hits_bed, genome, sd, args,
    ...
    # Write extended flank BED using the same direction as the main walk
...
def walk_step(step_num, query_fa, genome, outdir, sear_wd, args, csizes,
    ...
    # 0 ── Try extended flank extraction from previous step's hits ────────
    if (args.try_extended_first
        ...
        if ext_exts:
            ...
            # Return None as next_hits_bed: no new sear was run, so the next
            # step will perform a fresh genome search with the new consensus.
